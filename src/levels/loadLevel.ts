import type { GameWorld, SpawnEntry } from '@/ecs/Entity';
import {
  createSpawner,
  createGoal,
  createWisp,
...
export type EntityDescriptor =
  ...
  | { type: 'goal'; x: number; y: number; for: string }
  | { type: 'wisp'; x: number; y: number }
  | { type: 'redirect'; x: number; y: number; for: string; exits: { x: number; y: number; weight: number }[]; radius: number };
